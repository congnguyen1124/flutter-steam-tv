...
## StreamTV emulator-build patch

This directory vendors `video_player_avplay` 0.8.16 for StreamTV's Tizen 6.0 target. The upstream
package contains prebuilt PlusPlayer libraries for ARM only, so its native target fails at link time
on an x86 TV emulator before the application can start. This patch builds a no-op registrar on
non-ARM architectures and leaves the upstream implementation unchanged for ARM Samsung TVs.

The patch makes emulator builds usable for the application shell, navigation, and lifecycle. It
does not add emulator playback support: opening a stream produces the application's normal playback
error state. Only the upstream `armel/6.0` libraries are vendored because the application manifest
targets Tizen API 6.0.
